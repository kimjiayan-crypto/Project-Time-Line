...
<head>
    ...
    <title>營造工期排程生成器 V8.0 (知識庫整合版)</title>
    <style>
        /* --- 基礎樣式 (沿用 V7) --- */
        :root {
...
<header class="app-header">
    <div class="app-title">🏗️ 營造排程生成器 V8.0 (知識庫版)</div>
</header>
...
<div class="main-container">
    <aside class="sidebar">
        ...
        <div class="form-section">
            <h4>1. 基本與結構</h4>
            <div class="form-row">
                <div class="form-group"><label>工法</label><select id="method"><option value="CON">順打</option><option value="INV">逆打</option></select></div>
                <div class="form-group"><label>結構</label><select id="structure"><option value="RC">RC</option><option value="SRC">SRC</option><option value="SS">SS</option></select></div>
            </div>
            <div class="form-row">
                <div class="form-group"><label>地上(F)</label><input type="number" id="agf" value="15"></div>
                <div class="form-group"><label>地下(B)</label><input type="number" id="bgl" value="4"></div>
            </div>
            <div class="form-group" id="inv-opt" style="display:none;"><label>逆打轉換層</label><input type="number" id="trans_fl" value="1"></div>
        </div>
        <div class="form-section">
            <h4>2. 連續壁與開挖</h4>
            <div class="form-group"><label>連續壁單元數</label><input type="number" id="wall_units" value="80"></div>
            <div class="form-row">
                <div class="form-group"><label>開挖階數</label><input type="number" id="excav_steps" value="5"></div>
                <div class="form-group"><label>支撐層數</label><input type="number" id="brace_layers" value="4"></div>
            </div>
            <div class="form-group"><label>土方每日產能 (m³/day)</label><input type="number" id="excav_rate" value="500"></div>
        </div>
        <div class="form-section">
            <h4>3. 面積與環境</h4>
            <div class="form-group"><label>總樓地板面積 (m²)</label><input type="number" id="area_total" value="15000"></div>
            <div class="form-group"><label>開挖面積 (m²)</label><input type="number" id="area_excav" value="1200"></div>
        </div>
        <div class="form-section">
            <h4>4. 裝修與設備</h4>
            <div class="form-row">
                <div class="form-group"><label>正式電梯數</label><input type="number" id="elev_count" value="3"></div>
                <div class="form-group"><label>景觀規模</label><select id="landscape_type"><option value="STD">一般</option><option value="CMPX">量大</option></select></div>
            </div>
        </div>
        <button class="btn-primary" onclick="generateV8()">生成排程表</button>
    </aside>

    <main class="content-area">
        <div class="toolbar">
            <div id="stats-display">💡 點擊長條圖可查看詳細備註 (V8.0)</div>
...
<script>
    // ==========================================
    // 🛠️ 知識庫 (KNOWLEDGE BASE) - 可在此處更新備註
    // ==========================================
    const KNOWLEDGE_BASE = {
        "EXCAVATION": `
            <ul>
                <li><b>土方開挖能量：</b>第一層開挖若是鋪面用咬碎的方式，效率只有 <span class="highlight-red">400方/天</span>。</li>
                <li><b>逆打：</b>出土口受限，效率通常較順打低 (約70%)。</li>
                <li><b>工期計算：</b>體積 / 日產能 = 工作天數。</li>
            </ul>
        `,
        "BS_STRUCT": `
            <ul>
                <li><b>養護拆撐順序：</b>地下結構樓板完成後需<span class="highlight-red">養護7天</span>才能拆除支撐。</li>
                <li><b>趕工：</b>若趕工時可能會先壓試體，可爭取約3天工期 (即養護3-4天)。</li>
                <li><b>水箱蓋板：</b>含養護抓 50 天。</li>
                <li><b>一般樓層：</b>含養護抓 40 天。</li>
                <li><b>PC版：</b>應列在 B3 (或最底層) 版之子階。</li>
            </ul>
        `,
        "SUPPORT": `
            <ul>
                <li><b>拆除流程：</b>應考慮時間限制以確保安全性。</li>
                <li><b>限制：</b>如 AB 區做支撐要同一天。</li>
                <li><b>時差：</b>拆撐時差 7 天內尚可接受。</li>
                <li><b>邏輯：</b>拆除 N 層支撐前，N+1 層樓板必須養護完成。</li>
            </ul>
        `,
        "HOIST": `
            <ul>
                <li><b>啟動時機：</b>待結構蓋至 <span class="highlight-red">3～4樓</span> 時啟動安裝作業。</li>
                <li><b>安裝：</b>7 天。</li>
                <li><b>安檢：</b>21 天。</li>
                <li><b>拆除：</b>從第一部正式電梯裝好後 +50 天。</li>
            </ul>
        `,
        "ELEVATOR": `
            <ul>
                <li><b>安裝產能：</b>50天/台（一組人）。</li>
                <li><b>人力調度：</b>3台以上才會請兩組人。</li>
                <li><b>啟動條件：</b>應依據機房天花板在哪個樓層，該樓層完成後 <span class="highlight-red">21天</span> 才能安裝。</li>
            </ul>
        `,
        "FACADE": `
            <ul>
                <li><b>工期經驗值：</b>14樓的量體約 300~400 天，最多 500 天。</li>
                <li><b>啟動時機：</b>一般從上構 <span class="highlight-red">7F</span> 開始做外牆。</li>
                <li><b>內容：</b>包含放樣、泥作打底、磁磚/石材、填縫、清洗等。</li>
            </ul>
        `,
        "SCAFFOLD": `
            <ul>
                <li><b>20樓以下：</b>拆除約 30~45 天。</li>
                <li><b>20樓以上：</b>拆除約 50~60 天。</li>
                <li><b>三角架：</b>拆回一層大約兩周。</li>
            </ul>
        `,
        "LANDSCAPE": `
            <ul>
                <li><b>量少 (法定規範內)：</b>約 30~45 天。</li>
                <li><b>量大 (特殊設計)：</b>一般要在 1~3樓完成後才能完成，但最多不超過 60 天。</li>
                <li><b>順序：</b>通常在鷹架拆除後進行，避免高空落物風險。</li>
            </ul>
        `,
        "DEFAULT": "無特定備註，請參考一般工程規範。"
    };
    ...
    function showNote(kbKey, taskName) {
        ...
        document.getElementById('modalContent').innerHTML = `
            <span class="note-tag">分類代碼: ${kbKey}</span>
            ${content}
        `;
    ...
    function generateV8() {
        ...
        t = push('基地點交/鑑界', 21, 'cat-prep');
        
        let wallDays = Math.ceil(wallUnits * PARAMS.WALL_UNIT_DAYS);
        t = push(`連續壁 (${wallUnits}單元)`, wallDays, 'cat-prep', 'DEFAULT');

        if(method === 'CON') {
            t = push('中間樁打設', PARAMS.PILE_DAYS, 'cat-prep', 'DEFAULT');
            let depthAvg = 3.5;
            for(let i=1; i<=excavSteps; i++) {
                let vol = Math.round(areaExcav * depthAvg);
                let rate = (i===1) ? 400 : excavRate; // 知識庫規則: 第一層400
                let exDays = Math.ceil(vol / rate);
                if(exDays < 5) exDays = 5;
                t = push(`第${i}階開挖 (${vol}m³)`, exDays, 'cat-excav', 'EXCAVATION');
                if(i <= braceLayers) {
                    t = push(`第${i}層支撐施作`, 10, 'cat-excav', 'SUPPORT');
                }
            }
            t = push('PC/大底/水箱蓋板', PARAMS.BASE_SLAB_DAYS, 'cat-struct', 'BS_STRUCT'); // 知識庫: 水箱50天
            
            for(let b = bgl; b >= 1; b--) {
                let slabDays = (b === bgl) ? 0 : PARAMS.BS_SLAB_DAYS; 
                if(slabDays > 0) {
                    if(b < bgl) t = push(`拆除 B${b}層支撐`, 5, 'cat-excav', 'SUPPORT');
                    t = push(`B${b}F 結構工程`, slabDays - 7, 'cat-struct', 'BS_STRUCT');
                    t = push(`B${b}F 養護`, 7, 'cat-struct', 'BS_STRUCT'); // 知識庫: 養護7天
                }
            }
            genSuperStruct(t, structure, transFl, agf, stdArea);
        } else {
            // 逆打簡化 (但套用知識庫key)
            t = push('舊基清理', 30, 'cat-prep', 'DEFAULT');
            t = push('一階開挖', 25, 'cat-excav', 'EXCAVATION');
            t = push(`${transFl}F 轉換層結構`, 40, 'cat-struct', 'DEFAULT');
            let fork = t + 21; 
            let dt = fork;
            for(let b=1; b<=bgl; b++){
                let exD = Math.ceil((areaExcav*3.5)/(excavRate*0.7));
                let s = dt; let e = addWD(s, exD);
                taskList.push({name:`B${b}F 開挖`, start:s, end:e, days:exD, cat:'cat-excav', kbKey:'EXCAVATION'});
                dt = e;
                e = addWD(dt, 30);
                taskList.push({name:`B${b}F 結構`, start:dt, end:e, days:30, cat:'cat-struct', kbKey:'BS_STRUCT'});
                dt = e + 10;
            }
            taskList.push({name:'大底收尾', start:dt, end:dt+60, days:50, cat:'cat-struct', kbKey:'BS_STRUCT'});
        ...
        // Facade
        let facadeDur = (agf > 20) ? 500 : (agf > 14 ? 400 : 300); // 知識庫規則
        let facStart = time7F;
        let facEnd = addWD(facStart, facadeDur);
        taskList.push({name:'外牆裝修工程', start:facStart, end:facEnd, days:facadeDur, cat:'cat-finish', kbKey:'FACADE'});

        // Scaffold
        let scaffDur = (agf < 20) ? 40 : 60; // 知識庫規則
        let scaffStart = facEnd;
        let scaffEnd = addWD(scaffStart, scaffDur);
        taskList.push({name:'鷹架拆除', start:scaffStart, end:scaffEnd, days:scaffDur, cat:'cat-finish', kbKey:'SCAFFOLD'});

        // Perm Elevator
        let permElevStart = topEnd + 21; // 知識庫: 機房+21d
        let teams = elevCount > 3 ? 2 : 1;
        let permElevDur = Math.ceil(elevCount / teams) * 50; // 知識庫: 50d/台
        let permElevEnd = addWD(permElevStart, permElevDur);
        taskList.push({name:`正式電梯安裝(${elevCount}台)`, start:permElevStart, end:permElevEnd, days:permElevDur, cat:'cat-equip', kbKey:'ELEVATOR'});

        // Hoist
        let hoistInstStart = time4F; // 知識庫: 3-4F
        let hoistInstEnd = addWD(hoistInstStart, PARAMS.HOIST_INST + PARAMS.HOIST_INSP);
        taskList.push({name:'施工電梯安裝/安檢', start:hoistInstStart, end:hoistInstEnd, days:28, cat:'bar-hoist', kbKey:'HOIST'});
        
        let hoistRemStart = permElevEnd + 50; // 知識庫: 正式+50d
        let hoistRemEnd = addWD(hoistRemStart, 14);
        taskList.push({name:'施工電梯拆除', start:hoistRemStart, end:hoistRemEnd, days:14, cat:'bar-hoist', kbKey:'HOIST'});

        // Landscape
        let landDur = (landscapeType === 'CMPX') ? 60 : 40;
        let landStart = scaffEnd - 20; 
        let landEnd = addWD(landStart, landDur);
        taskList.push({name:'景觀工程', start:landStart, end:landEnd, days:landDur, cat:'cat-finish', kbKey:'LANDSCAPE'});

        // Permit
        let fireStart = scaffEnd + 5;
        let fireEnd = addWD(fireStart, 45);
        taskList.push({name:'消防檢查', start:fireStart, end:fireEnd, days:45, cat:'cat-prep', kbKey:'DEFAULT'});
        let permitEnd = addWD(fireEnd, 60);
        taskList.push({name:'★ 取得使照', start:fireEnd, end:permitEnd, days:60, cat:'cat-permit', kbKey:'DEFAULT'});
    ...
    function genSuperStruct(startT, struct, sF, eF, area, isParallel=false) {
        ...
        if(struct === 'SS') {
            ...
            while(curF <= eF) {
                ...
                let e = addWD(curT, ed);
                taskList.push({name:`鋼構吊裝 ${curF}-${curF+n-1}F`, start:curT, end:e, days:ed, cat:'cat-steel', kbKey:'DEFAULT'});
                let dd = Math.ceil(n*12*areaMul);
                let ds = curT + 10; let de = addWD(ds, dd);
                taskList.push({name:`Deck版 ${curF}-${curF+n-1}F`, start:ds, end:de, days:dd, cat:'cat-struct', kbKey:'DEFAULT'});
                curT = e; curF += n;
            }
        } else {
            for(let f=sF+1; f<=eF; f++) {
                ...
                taskList.push({name:`${f}F 結構`, start:curT, end:e, days:dur, cat:'cat-struct', kbKey:'DEFAULT'});
                curT = e;
            }
        }
    }
    ...
    function renderV8() {
        ...
        const cn = document.createElement('div'); cn.className='gantt-corner'; cn.textContent='工項名稱'; hRow.appendChild(cn);
